...
if not show_all:
    ...
    st.session_state.selected_code = top_stock["종목코드"]
    st.session_state.selected_name = top_stock["종목명"]
    st.info(f"🚀 자동으로 {top_stock['종목명']} ({top_stock['종목코드']}) 차트로 이동합니다...")
...
# ------------------------------------------------
st.markdown("---")
st.caption("💡 상위 5개는 자동 이동, 전체 보기에서는 클릭 시 이동합니다.")
# -*- coding: utf-8 -*-
import streamlit as st
import pandas as pd
from supabase import create_client
